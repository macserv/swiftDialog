        ...
        VStack {
            ...
            HStack {
                Picker("ck-textalignment".localized, selection: $observedData.args.messageAlignment.value) {
                    Text("").tag("")
                    ForEach(observedData.appProperties.alignmentStates.keys.sorted(), id: \.self) {
                        Text($0)
                    }
                }
                .onChange(of: observedData.args.messageAlignment.value) {
                    observedData.appProperties.messageAlignment = observedData.appProperties.alignmentStates[$0] ?? .leading
